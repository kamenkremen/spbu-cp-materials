...
#align(center,
  text(size: 14pt)[
    #table(
      ...
      $f_4(n) = Sigma_(i=1)^(n)i$,  $f_4(n) = O(n^2)$,
      $f_5(n) = n/3$,               $f_5(n) = O(n)$,
...
- Увеличиваем $r$ пока сумма $<= k$
- Если сумма равна $k$, то возможно ответ найден
- Пока сумма $> k$, увеличиваем $l$
...
- $p_2 = a_1 + a_2$
- $dots$
- $p_k = Sigma_(i = 0)^(k) a_i = p_(k-1) + a_k$
...
- Сумму на отрезке $[0, r]$ мы знаем(она равна $p_(r)$)
- Сумму на отрезке $[0, l - 1]$ мы знаем(она равна $p_(l-1)$)
