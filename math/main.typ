...
Так же можно вычислить по формуле $C_n^k = display(n!/(k! dot (n-k)!))$
...
$(a + b)^n = C(n, 0)dot a_n dot b_0 + C(n, 1)dot a_n - 1 dot b_1 + ... + C(n, n) dot a_0 dot b_n$

==
Функция позволяющая посчитать $C_n^k$ за $O(k)$
```cpp
...
==
Предподсчёт биномиальных коэфицентов за $O(n^2)$
```cpp
const int MAXN = 1001;
...
=== Задача: Последовательности со строгими неравенствами

Дано число $n$ и массив из $k$ пар $("pos"_1, "val"_1), ("pos"_2, "val"_2), ..., ("pos"_k, "val"_k)$.

Сколько существует последовательностей $a_1, a_2, ..., a_n$ таких, что:
- $1 <= a_1 <= m$ для всех $i$
- $a_1 < a_2 < ... < a_n$ (строго возрастает)
- $a_("pos"_i) = "val"_i$ для всех $i$ от $1$ до $k$
...
$C_n = C_0 dot C_(n-1) + C_1 dot C_(n - 2) + ... + C_(n - 1) dot C_0$
...
$C_n = display(1/(n + 1))dot C_(2n)^n = С_(2n)^n - C_(2n)^(n-1)$
...
#definition[
Определение *НОД*: $d=max{c in N : c divides a,c divides b}$
]

#definition[
Определение *НОД*: $m=min{c in N : a divides c, b divides c}$
]
...
- Еще задачки: #link("https://3.shkolkovo.online/catalog/2887?Page=2&SubTheme=2888")[в курсе школково]
== Алгоритм Евклида 
...
- Чтобы почувствовать: #link("https://vmath.ru/vf5/numtheory#linejnoe_predstavlenie_nod")[Линейное представление НОД]
...
== Задача (CF - 1700 rate): 
#link("https://codeforces.com/problemset/problem/2065/G")[Div.4G Скибидус и ложь]

  Целое число x считается полупростым, если его можно записать в виде $p dot q$, где $p$ и $q$ - (не обязательно разные) простые числа. Например, $9$ является полупростым, так как его можно записать как $3 dot 3$, а $3$ является простым числом. Скибидусу был дан массив $a$, содержащий $n$ целых чисел. Он должен сообщить количество пар $(i,j)$ таких, что $i<=j$ и $lcm(a_i,a_j)$ является полупростым.
...
==
Возможно на codeforces вы встречались с формулировками по типу:
...
- Умножение: $(a dot b)mod m = ((a mod m) dot (b mod m)) mod m$
...
```
Но что же делать с делением? Попробуем предсавить деление чуть иначе. $a\/b = a dot b^(-1)$. Тогда нам достаточно найти число обратное данному, а в конетексте модульной арифметики достаточно найти число эквивалентое по модулю обратному. 
...
$
a^n = cases(
a^(n/2)dot a^(n/2) "if" n "is even", 
a^(n-1)dot a "if" n "is odd", 
)
$
])
#remark[
...
Другое решение это использовать расширенный алгоритм Евклида. В выражение $A dot x + B dot y = 1$ подставим в качестве $A$ и $B$ соответственно $a$ и $m$: $a dot x + m dot y = 1$. Одним из решений уравнения и будет $a^(-1)$, потому что если взять уравнение по модулю $m$, то мы получим:

$a dot x + m dot y = 1 <=> a dot x equiv 1 | :a <=> x equiv a^(-1) (mod m)$.
